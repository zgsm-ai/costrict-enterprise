...
/**
 * Get context information for code completion
 * @param {context.Context} ctx - Context for request cancellation and timeout
 * @param {string} clientID - Client identifier for the request
 * @param {string} projectPath - Path to the project root
 * @param {string} filePath - Path to the file being analyzed (relative to project)
 * @param {string} prefix - Code content before cursor position
 * @param {string} suffix - Code content after cursor position
 * @param {string} importContent - Import statements for the file
 * @param {http.Header} headers - HTTP headers for the requests
 * @returns {string} Returns formatted context information with comments
 * @description
 * - Validates input parameters and returns empty string if invalid
 * - Constructs full file path with proper Windows path handling
 * - Extracts semantic search content from prefix (last 4 lines)
 * - Builds code snippets for definition search
 * - Performs multi-source context retrieval (definition, semantic, relation)
 * - Parses and combines results from all search types
 * - Adds comments to the final context result
 * @example
 * context := client.GetContext(ctx, "client-id", "/project", "src/file.go",
 *     "func main() {", "}", "import fmt", headers)
 */
...
/**
 * Search for code definitions in the codebase
 * @param {context.Context} ctx - Context for request cancellation and timeout
 * @param {string} clientID - Client identifier for the request
 * @param {string} codebasePath - Path to the codebase being searched
 * @param {string} filePath - Path to the file containing the code snippet
 * @param {string} codeSnippet - Code snippet to search for definitions
 * @param {http.Header} headers - HTTP headers for the request
 * @returns {ResponseData, error} Returns search result data and error if any
 * @description
 * - Constructs request parameters for definition search
 * - Makes HTTP GET request to definition URL configured in context
 * - Uses API client to execute the request
 * - Returns response data or error if request fails
 * @example
 * result, err := client.searchDefinition(ctx, "client-id", "/codebase", "file.go", "func test()", headers)
 */
 ...
 * @returns {ResponseData, error} Returns search result data and error if any
 * @description
 * - Constructs request parameters for semantic search
 * - Includes top-K and score threshold from configuration
 * - Makes HTTP POST request to semantic URL configured in context
 * - Uses API client to execute the request
 * - Returns response data or error if request fails
 * @example
 * result, err := client.searchSemantic(ctx, "client-id", "/codebase", "database connection", headers)
 */
...
/**
 * Search for code relations and dependencies
 * @param {context.Context} ctx - Context for request cancellation and timeout
 * @param {string} clientID - Client identifier for the request
 * @param {string} codebasePath - Path to the codebase being searched
 * @param {string} filePath - Path to the file containing the code snippet
 ...
 * @returns {ResponseData, error} Returns search result data and error if any
 * @description
 * - Constructs request parameters for relation search
 * - Includes max layer and include content flags from configuration
 * - Makes HTTP GET request to relation URL configured in context
 * - Uses API client to execute the request
 * - Returns response data or error if request fails
 * @example
 * result, err := client.searchRelation(ctx, "client-id", "/codebase", "file.go", "func test()", headers)
 */
